...
配置页支持新建、查看、编辑和删除 Skill，也可以扫描最近 30 天的成功历史任务并生成待审核候选：

- 精确扫描只合并命令结构完全一致的重复流程。
- 语义扫描会把脱敏后的任务标题和命令摘要发送给已配置模型；模型只能返回任务 ID 分组，不能生成命令或 Skill YAML。
- 语义分组后的命令由本地确定性编译器处理，当前只允许把一组独立的绝对路径差异和一组整数差异参数化；步骤数、步骤结构或其他命令内容不一致时直接拒绝候选。
- 模型不可用、超时或响应无效时自动保留精确扫描结果，不影响页面操作。
- 候选必须经过安全预览和人工批准；批准后只创建为停用 Skill，仍需人工核对成功判据并手动启用。
