...
def _extract_params(
    ...
    for param in skill.params:
        ...
        if param.enum and value not in param.enum:
            value = None
        if param.pattern and value and not re.search(param.pattern, str(value)):
            value = None
...
def _render_steps(skill: TemplateSkill, params: dict[str, Any]) -> list[dict[str, str]]:
    steps: list[dict[str, str]] = []
    ...
    for target in render_targets:
        ...
        for step in skill.steps:
            steps.append(
                {
                    "command": _render_template(step.command, string_params),
                    "intent": _render_template(step.intent, string_params),
                    "explanation": _render_template(step.explanation, string_params),
